		...
		if (ADDRESS_UNAVAILABLE == check)
		{
			if (0 == preferred_bit)
			{
				...
				if (!IsFull(node->children[!preferred_bit])
				{
					...
					check = GetSmallestAvailable(node->children[preferred_bit], 
												 _ip, level - 1);
				}
				else
				{
...
static status_t GetSmallestAvailable(
	dhcp_node_t *node,
	ip_t _ip,
	size_t level
)
{
	if (level > 0)
	{
		...
		check = GetSmallestAvailable(node->children[bit], _ip, level - 1);
