...
static uint64_t SetBusy(uint64_t board, int position);
static uint64_t InitAvailablePositions(
	uint64_t board, 
	position_t buffer[], 
	int position
);
...
static int IsAvailable(uint64_t board, int position);
...
)
{
	...
	if (!IsEndOfBoard(step))
	{
		...
		uint64_t i = 0, available_directions = 0;
...
)
{
	...
	uint64_t available_directions = 0;
...
static uint64_t SetBusy(uint64_t board, int position)
{
	return board | ((uint64_t)1 << position);
}

static int IsAvailable(uint64_t board, int position)
{
	return (0 == ((uint64_t)1 & (board >> position)));
}
